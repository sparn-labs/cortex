...
program.configureHelp({
  formatHelp(cmd, helper) {
    ...
    lines.push(`  interactive, graph, search, plan, exec, verify, docs, dashboard`);
    lines.push('');
    lines.push('Options:');
...
program
  .command('scan [path]')
  .description('Quick code analysis — quality, security, test coverage')
  .option('--focus <categories>', 'Focus on: quality,security,tests (comma-separated)')
  .option('--json', 'Output as JSON')
  .option('--output <file>', 'Write report to file')
  .option('--verbose', 'Show detailed findings')
  .option('--audit', 'Strict audit mode')
  .action(async (path, options) => {
    ...
    try {
      ...
      if (options.output) {
        const { writeFileSync } = await import('node:fs');
        writeFileSync(options.output, options.json && result.json ? result.json : result.markdown);
...
program
  ...
  .action(async () => {
    ...
    console.log(synapseViolet('\n  Commands: graph | search | plan | exec | verify | docs\n'));
  });
